...
Все брендовые ролики Monty, отрендеренные из Remotion-проекта `~/monty-howto-video` (движок общий, `src/*.tsx` + `content.json`). Каждый ролик в двух ориентациях: `-horizontal` = 1920×1080 (YouTube / X / сайт), `-vertical` = 1080×1920 (Reels / Shorts / TikTok / Stories). Кодек H.264 + AAC, готовы к постингу как есть.

| Папка | Ролик | Длина | Языки | Про что |
|---|---|---|---|---|
| `video/what-monty/` | WhatMonty (hero) | ~22 c | en ru | Промо фич в стиле Submagic, 5 битов. Стоит героем на monty.fast и partner.monty.fast |
...
Перерендерить или добавить язык: правится `content.json` в `~/monty-howto-video`, рендер через Remotion, готовые файлы кладутся сюда. Промежуточные / пре-языковые версии в кит не берём - только финальные, что стоят на проде.

## Плейсхолдеры (заменить перед раздачей партнёру)

- `ТВОЙ_КОД` / `YOUR_CODE` → персональный код партнёра
- QR-карты сгенерены на `YOUR_CODE` - перегенерить под каждого (команда ниже)

## Перегенерить QR под партнёра
...
SVG = источник (открывать в Figma с установленным Space Grotesk). PNG:
```bash
pip3 install cairosvg
python3 -c "import cairosvg; cairosvg.svg2png(url='story-frame-ru.svg', write_to='story-frame-ru.png', output_width=1080)"
```
Без установленного Space Grotesk текст ляжет системным шрифтом - ставить шрифт перед экспортом или рендерить в Figma.

## Что ещё нужно (не генерится, задача дизайн/видео-команде)

Бренд-гайд просит маскота в позах-состояниях (visual-identity.md §5): Думает / Монтит / Постит / Празднует / Спит-на-автопилоте. Для них нужна иллюстрация.

- [ ] Маскот в 5 состояниях (бюст-версия)
- [ ] 3-5 демо-роликов «Made with Monty» для реюза
- [ ] Гифки процесса (заливка → готовый ролик), poof-анимация
- [ ] Финальные PNG-экспорты с зашитым Space Grotesk

Условия партнёрки: страница регистрации партнёра на monty.fast.
Тексты кита: `../AFFILIATE-PROMOKIT.md` (RU), `../AFFILIATE-PROMOKIT-EN.md` (EN).
